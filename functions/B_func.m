function B = B_func(xi,lambda,mu)
% Calculates the integrand for B_kj, (K(xi)+D(mu))^-1*xi^-1
%
...
if N == 1
    ...
    K = repmat(eye(N),1,1,length(xi)).*reshape(xi.^2,1,1,length(xi))+...
        diag([lambda(1)^2; 2*lambda(2:end-1).^2; lambda(end)^2])+...
        diag(-lambda(1:end-1).^2,1)+diag(-lambda(2:end).^2,-1);
    B = pageinv(K+diag(mu))./reshape(xi,1,1,length(xi));
end
